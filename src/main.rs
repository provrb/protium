...
fn main() {
    ...
    ecm.queue_transmission("hello".as_bytes().to_vec(), false)
    ...
    tcm.set_on_complete_receive_callback(|node_id, received_bits| {
        ...
        let Ok(frame) = Frame::from_encoded_bits(received_bits).inspect_err(|e| {
            printerr!("frame::from_encoded_bits returned error {e}");
        }) else {
            return;
        };
